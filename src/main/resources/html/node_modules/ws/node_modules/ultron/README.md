...
```js
ultron.on('event-name', com.lee.server.retrofit.handler, { custom: 'function context' });
```
...
```js
ultron
.on('event-name', com.lee.server.retrofit.handler)
.on('another event', com.lee.server.retrofit.handler);
```
...
```js
ultron
.once('event-name', com.lee.server.retrofit.handler, { custom: 'this value' })
.once('another event', com.lee.server.retrofit.handler);
```
