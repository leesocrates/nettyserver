...
public abstract class DualAbstractHandler<T, RouteLike extends DualMethodRouter<T, RouteLike>>
	 ...
	 *            you want.
	 */
	protected abstract void routed(ChannelHandlerContext ctx, Routed routed)
			throws Exception;
	...
	protected void messageReceived(ChannelHandlerContext ctx, HttpRequest req)
		...
		Routed routed = new Routed(jrouted.target(), jrouted.notFound(), req,
				qsd.path(), jrouted.params(), qsd.parameters());
		routed(ctx, routed);
	}
